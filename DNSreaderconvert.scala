...
object CommonUsageExamples {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val FILENAME: String = "dump-dns-random-12K.pcap"
    val errbuf = new java.lang.StringBuilder()
    val pcap: Pcap = Pcap.openOffline(FILENAME, errbuf)
    if (pcap == null) {
// Error is stored in errbuf if any
    ...
    pcap.loop(
      10,
      new JPacketHandler[java.lang.StringBuilder]() {
        ...
        /**
          ...
          */
        def nextPacket(packet: JPacket, errbuf: StringBuilder): Unit = {
          /*
          ...
          if (packet.hasHeader(Tcp.ID)) {
            ...
            println("tcp.dst_port=%d%n", tcp.destination())
            println("tcp.src_port=%d%n", tcp.source())
            println("tcp.ack=%x%n", tcp.ack())
          }
          /*
          ...
          if (packet.hasHeader(tcp)) {
            println("tcp header::%s%n", tcp.toString)
          }
          /*
          ...
          println("frame #%d%n", packet.getFrameNumber)
        }
      },
      errbuf
    )
    ...
    val packet: PcapPacket = new PcapPacket(JMemory.POINTER)
    val tcp: Tcp = new Tcp()
    for (i <- 0.until(5)) {
      pcap.nextEx(packet)
      if (packet.hasHeader(tcp)) {
        println("#%d seq=%08X%n", packet.getFrameNumber, tcp.seq())
      }
    }
    /*
    ...
    val flows: Map[JFlowKey, JFlow] = new HashMap[JFlowKey, JFlow]()
    for (i <- 0.until(50)) {
      pcap.nextEx(packet)
      val key: JFlowKey = packet.getState.getFlowKey
      /*
    ...
    for (flow <- flows.values) {
      ...
      if (flow.isReversable) {
        val forward: List[JPacket] = flow.getForward
        for (p <- forward) {
          println("%d, ", p.getFrameNumber)
        }
        println()
        val reverse: List[JPacket] = flow.getReverse
        for (p <- reverse) {
          println("%d, ", p.getFrameNumber)
        }
      }
    ...
    val superFlowMap: JFlowMap = new JFlowMap()
    /*
    ...
    println("superFlowMap::%s%n", superFlowMap)
    /*
